...
return view.extend({
    ...
    render: function (load_result) {
        ...
        let asset_file_status = _('WARNING: at least one of asset files (geoip.dat, geosite.dat) is not found under /usr/share/xray. Xray may not work properly. Install <code>luci-app-xray-geodata</code>, then open Xray (geodata) and update GeoData from Loyalsoldier/v2ray-rules-dat.');
        if (geoip_existence) {
